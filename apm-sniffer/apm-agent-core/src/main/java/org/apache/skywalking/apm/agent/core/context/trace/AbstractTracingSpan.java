...
public abstract class AbstractTracingSpan implements AbstractSpan {
     ...
     * <p>
     * 用于当前Span 指定自己所在的Segment 的第一个Segment.除非这个Span是所在Segment是整个链路上的第一个Segment
     */
    protected List<TraceSegmentRef> refs;

    /**
     * Tracing Mode. If true means represents all spans generated in this context should skip analysis.
     * 追踪模式。如果 true 表示在此上下文中生成的所有span应跳过分析。
     */
    ...
    public AbstractTracingSpan log(Throwable t) {
        ...
        logs.add(new LogDataEntity.Builder().add(new KeyValuePair("event", "error"))
                .add(new KeyValuePair("error.kind", t.getClass().getName())) // 异常名
                .add(new KeyValuePair("message", t.getMessage())) // 异常堆栈
